...
// DataManager implementation. It separates the mutable draft schema (a
// single document, overwritten by SetSchema) from the immutable published
// history (append-only snapshots produced by Publish and Activate).
//
...
type SchemaManager interface {
	// Draft — a single mutable document.

	// SetSchema overwrites the current draft schema. The draft is never
	// versioned; only published snapshots carry version numbers.
	// ValidateSchema is NOT called here — invalid drafts are permitted
	// until Publish.
	SetSchema(ctx context.Context, s schema.Schema) error

	// GetSchema returns the current draft schema. Returns
	...
	// yet.
	GetActive(ctx context.Context) (schema.Schema, error)

	// GetVersion returns a specific published version. Returns
	// ErrSchemaNotFound if the version does not exist.
	GetVersion(ctx context.Context, version int) (schema.Schema, error)

	// ListVersions returns all published versions in ascending version
	// order. Includes both active and inactive versions.
	ListVersions(ctx context.Context) ([]schema.Schema, error)
}
