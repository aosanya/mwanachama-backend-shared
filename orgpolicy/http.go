...
func PutOrgPolicy(repo Repository, callerID func(*http.Request) string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !raw["public_address_cap"] && !raw["structure_membership_cap"] &&
			!raw["free_text_max_length_cap"] {
			writeErr(w, http.StatusBadRequest,
				"name a setting to change: public_address_cap, structure_membership_cap, "+
		...
		if raw["structure_membership_cap"] && in.StructureMembershipCap == nil {
			writeErr(w, http.StatusBadRequest, "structure_membership_cap cannot be null")
			return
		}
		...
		if in.StructureMembershipCap != nil {
			p.StructureMembershipCap = *in.StructureMembershipCap
		}
